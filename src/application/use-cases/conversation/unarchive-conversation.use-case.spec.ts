import { UnarchiveConversationUseCase } from './unarchive-conversation.use-case';
import { Conversation, type ConversationRepository } from '../../../domain';

describe('UnarchiveConversationUseCase', () => {
  ...
  it('returns CONVERSATION_NOT_FOUND when conversation does not exist', async () => {
    ...
    const sut = new UnarchiveConversationUseCase(conversationRepository);
  ...
  it('unarchives and saves the conversation for the owner', async () => {
    const conversationRepository = makeConversationRepository();
